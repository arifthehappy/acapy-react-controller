...
export const CreateCredentialDefinition = () => {
  ...
  const [revocationRegistrySize, setRevocationRegistrySize] = useState(CRED_DEF_DEFAULTS.revocation_registry_size);
  const [supportRevocation, setSupportRevocation] = useState(CRED_DEF_DEFAULTS.support_revocation);
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setSupportRevocation(CRED_DEF_DEFAULTS.supportRevocation);
    }
  });
...
const handleSubmit = (e: React.FormEvent) => {
  ...
  if (schemasLoading) {
    return <div className="flex justify-center items-center p-8">Loading schemas...</div>;
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white p-6 rounded-lg shadow-md space-y-6">
      ...
      <div className="flex items-center space-x-2">
        ...
        />
        <label htmlFor="supportRevocation" className="text-sm font-medium text-gray-700">
          Support Revocation
        </label>
      </div>
        ...
        {supportRevocation && (
            ...
            <input
                ...
                onChange={(e) => setRevocationRegistrySize(parseInt(e.target.value))}
                className="w-full p-2 border rounded-md focus:ring-indigo-500 focus:border-indigo-500"
                required
            />
            </div>
      ...
      {supportRevocation && (
        <div className="flex items-center space-x-2 bg-yellow-50 p-4 rounded-md">
          ...
          <p className="text-sm text-yellow-700">
            Enabling revocation will require additional setup for the tails file server.
          </p>
        </div>
      )}
      ...
      >
        <Plus size={20} />
        <span>{mutation.isPending ? 'Creating...' : 'Create Credential Definition'}</span>
      </button>

      {mutation.isError && (
        <div className="mt-4 p-4 bg-red-50 rounded-md">
          <p className="text-sm text-red-700">Error creating credential definition</p>
        </div>
      )}
    </form>
  );
